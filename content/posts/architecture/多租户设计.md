...
单个数据源下面有多个schema, 一个schema对应一个租户,   
jdbcurl不指定dababase, 运行时根据上下文use database的方式切换数据库  
或者sql里表明加上库名 from 库名.表名 
切换的时机可以使用mybatis拦截器，这样是每个sql前面都会加.  
可以优化成获取连接后第一个sql前面加上use语句或者动态修改 from table 改成 schema.table  
